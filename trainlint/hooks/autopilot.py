#!/usr/bin/env python3
"""Autopilot gate — keep driving /trainlint:execute-and-report until a HUMAN is needed.
...
tells the agent to invoke /trainlint:execute-and-report again. That is the loop.
...
def _enabled():
    return os.environ.get("TRAINLINT_AUTOPILOT", "").strip().lower() in ("1", "on", "true", "yes")


def _cap():
    try:
        return max(1, int(os.environ.get("TRAINLINT_AUTOPILOT_MAX", "").strip() or _DEFAULT_MAX))
    except Exception:
        return _DEFAULT_MAX
...
_SYS = (
    "You are the AUTOPILOT GATE for an ML-research planning loop (trainlint). After each "
    "/trainlint:execute-and-report (or /trainlint:plan) turn you decide whether the loop may "
...
def check(data):
    ...
    try:
        ...
        return (
            f"[trainlint autopilot ▸ {nxt}/{cap}] No human review, GPU/compute, or blocking "
            f"decision is needed right now.{why}\n\n"
            "Continue autonomously: invoke the /trainlint:execute-and-report skill to drive the "
