#!/usr/bin/env python3
"""hansard router — the orchestrator.
...
def decide(data):
    ...
    if event in ("Stop", "SubagentStop"):
        ...
        # blocking the stop with a "run /hansard:execute-and-report" reason.
    ...
    # the deliberate quiz built into /hansard:plan over the plan's decisions, and the `concept-gap-quiz`
    ...
    # (a freshly-registered project before /hansard:plan). Drop it — the rule isn't ready,
...
def main():
    try:
        ...
        if os.environ.get("HANSARD_HOST") or os.environ.get("TRAINLINT_HOST", "").lower() == "kimi":
