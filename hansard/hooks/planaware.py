...
def _action_is_foreign(data):
    """True if this action edits a file inside a tree marked with a `.hansard-foreign`
    file — a repo that is NOT a hansard-managed training project (e.g. hansard-builder),
    ...
    for t in paths:
        ...
        for anc in (rp, *rp.parents):
            try:
                if (anc / ".hansard-foreign").is_file():
                    return True
            except Exception:
                continue
    return False
...
def assess(data):
    ...
    if planlib is None:
        return [], []
    # FOREIGN EXEMPTION: an edit inside a tree marked .hansard-foreign is not a managed
            ...
            if not _seen_then_mark(session, did):
                items.append({"level": "escalate", "sticky": True, "plan_decision": did,
                              "message": (f"🚦 GATE — you're about to do {d.get('phase')}-stage work on "
                                          ...
                                          f"Quiz it first: `/hansard:plan {did}`.")})
            continue
        # soft status reminders — deduped once per (session, decision)
        if _seen_then_mark(session, did):
            continue
        gate = ("" if mastered
                else f" (you haven't walked this decision in quiz yet — `/hansard:plan {did}`)")
        if status == "open":
