...
`/hansard:plan` is the single entry point — it registers a new project AND plans it. Before
...
5. **Quiz me** — once the decisions are written, walk them right here (this is the quiz; there is no
   separate `/hansard:quiz` command). Use `research/progress.py` for the mastery bookkeeping:
...
6. **The closing REPORT — explain it like a person, end in motion.** When the decisions are written,
   ...
   - **Everything else** — one line: "<M> still open across <phase>→<phase> — ask about any, or
     re-run `/hansard:plan <id>` to drill one." A pointer, not a dump.
   ...
   **End by rendering the report AND putting it on my phone.** After the prose report, run
   ...
   `PHONE:` (how to deliver it). `/hansard:plan` and `/hansard:execute-and-report` BOTH close the
...
`/hansard:plan` is the ONLY plan command. For a normal project, do the foreground flow above. If
