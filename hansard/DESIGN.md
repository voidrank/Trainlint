...
- **Guidance / a static prompt (CLAUDE.md).** It is *persuasion you can ignore* — the model sees
  ...
  **blind** to the specific diff. And it can **enforce nothing.** A hook delivers the *same
  knowledge* but just-in-time, action-aware, and able to deny — so Hansard keeps the rules as
...
- **A skill / command.** It must be **invoked.** But the whole problem is that *you don't know
  ...
  must watch *ambiently*, unbidden. (Where on-demand DOES fit — visualize, quiz, init — Hansard
...
1. **Prefilter is default-open** (`prefilter.py`): only drop things that can be **proven harmless** (pure reads, docs, self-edits);
   ...
   **silently miss** dangerous actions that didn't match a keyword, making all the downstream judgment pointless.
   ...
   > **Foreign trees opt out the same way.** A repo that isn't *this* plugin but is still steeped in training vocabulary (e.g. `hansard-builder`,
   > which mines training-failure principles from commit histories) drops a `.hansard-foreign` file at its root; `planaware._action_is_foreign`
...
2. bind it to your session: `/hansard:use <new project>` (or set `HARNESS_PROJECT`).
...
flag and unwired from the router): the deliberate quiz built into `/hansard:plan`, walking the plan's decisions,
