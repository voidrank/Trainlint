...
# x-hansard-relay: <secret>; the cloud worker already Google-authenticated the viewer, so they
...
LOGIN_HTML = ("<!doctype html><meta name='viewport' content='width=device-width,initial-scale=1'>"
              ...
              "<h3>Hansard report</h3><input type='password' name='p' placeholder='password' autofocus "
...
# This backend trusts the x-hansard-relay header exactly like /chat's admin path and just performs
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _auth(self):
        ...
        sec = _relay_secret()
        if sec and _secrets.compare_digest(self.headers.get("x-hansard-relay", ""), sec):
