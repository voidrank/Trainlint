export const meta = {
  name: 'plan-engine',
  description: 'Internal engine for /hansard:plan (not a standalone command). The big-codebase offload path: gather full context in parallel, decompose into decisions, and WRITE plan.<name>.jsonl (+ fill the facts files). The write is guaranteed by the script, not left to the model to remember.',
  phases: [
...
if (!project || !root) {
  ...
  const r = await agent(
    'Find the hansard plugin and its active project. ' +
    ...
    '~/.claude/plugins/cache/hansard/*/ and any Hansard checkout). ' +
...
const planHeader =
  ...
  `# transferable PRINCIPLE that governs it. Written by /hansard:plan.\n` +
