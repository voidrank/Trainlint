...
`/hansard:plan` runs in the **foreground** by default (the agent owns the journey — it stays engaged
...
- **report** — when the tree gains a *real* search (branching or a wall), nudge to run
  `/hansard:execute-and-report` to render + send the picture.
- harvest runs on `PreCompact`/`SessionEnd`.

Quizzing has two paths: **deliberate** (the quiz built into `/hansard:plan`, walking the plan's
decisions) and
...
`/hansard:plan <name>` — it registers the project (thin scaffold), reads the code, fills the facts,
writes the decisions + goal + glossary, then quizzes you, all in one flow. Then drive it with
`/hansard:execute-and-report`. See `project.example.json` / `research/*.example.*` as worked examples.
